...
def admin_add_product(request):
    ...
    categories = crocsCategory.objects.all()
    ...
    if request.method == 'POST':
        ...
        if 'add_category' in request.POST:
            try:
                ...
                if category_name:
                    # Create new category
                    new_category = crocsCategory(name=category_name)
                    new_category.save()
        ...
        # Regular form submission for adding a product
            ...
            crocs_category_id = request.POST.get('crocs_category')
            gender_id = request.POST.get('gender')
            priority = request.POST.get('priority', 0)
            
            try:
                # Get the ShoeCategory object
                crocs_category = crocsCategory.objects.get(id=crocs_category_id)
    ...
    available_sizes = [4,5,6, 7, 8, 9, 10, 11]
...
def admin_edit_product(request, product_id):
    ...
    if request.method == 'POST':
        try:
            ...
            # Foreign keys
            product.crocs_category = crocsCategory.objects.get(id=request.POST.get('crocs_category'))
...
def delete_category(request, category_id):
    category = crocsCategory.objects.filter(id=category_id).first()  # Check if the category exists
...
def register_view(request):
    if request.method == 'POST' and 'register' in request.POST:
        try:
            ...
            send_mail(
                'LOFT Registration',
                '🎉 Welcome to  Easy walkers World! Your account has been created successfully.',
...
def crocs_view(request):
        ...
        return render(request,'user/crocs.html',context)
